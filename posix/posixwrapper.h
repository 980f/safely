...
#include "logger.h"
/**
 * handy things for wrapping a POSIX C API with a C++ class, if only for errno management. Fairly recent errno implementations are per-thread, but even so one has to store away the errno after every operation that affects it.
 * This class is handy for the latter.
 *
...
public:
   ...
   *  this updates errornumber member on each call, but only emits a system log message if the errno has changed since the last time a message was sent. See stifleEAGAIN for additional tuning of error logging */
public: // so that we can merge errors from functions called outside of the object which still use the same reporting mechanism
  ...
  bool
  isOk(bool againIsOk = true) const noexcept {
  ...
  /** set @param target with @param value, if value is -1 (all ones, ~0) then
   * call failure(errno), @returns whether value!=~0. */
  template <typename Integrish, typename Other>
  bool
  okValue(Integrish &target, Other value) {
...
public:
  ...
  /** replaces what was once 'failure' as that name works better for another purpose. if @param passthrough is true then errno is recorded else errornumber is left to a previous value.
   * @returns passthrough. */
  bool
  setFailed(bool passthrough);
}; // class PosixWrapper
