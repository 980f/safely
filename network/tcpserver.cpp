...
TcpServer::TcpServer(int port, int interfaceIP, int backlog): server(interfaceIP, port),
  backlog(backlog),
  connects(0),
  disconnects(0),
  listening(false) {
...
TcpServer::TcpServer(): TcpServer(0, 0,0){}
...
TcpServer::ServerSocket::ServerSocket(uint32_t remoteAddress, int port): TcpSocketBase(~0, remoteAddress, port) {
  //#nada
}
