...
static gunichar parse4hex(const char *&mark){
  gunichar kanjiuni(0);
...
static gunichar parseUPlus(const char *&mark){
  mark+=2;//skip "U+"
...
static const Range<gunichar> CjkUnified(0x9fff,0x4e00);
...
struct KanjiLookup:public ArtFile {
  ...
  void addReading(Storable&set,const char *textkey,gunichar kanjicode){
  ...
  void addReading(Storable&set,const Glib::ustring &yomi,gunichar kanjicode){
  ...
  static gunichar euc2uni(u16 euc){
  ...
  static bool isHiragana(gunichar c){//duplicate in Romaji
 ...
 //todo:1 debate whether punctuation should be included. */
  static bool isKatakana(gunichar c){
...
*/
  struct HanziGroup {
    ...
    static int find(const HanziGroup*oldone,gunichar unicode){
  ...
  HanziGroup &groupFor(gunichar hanzi){
  ...
  void flagGroup(gunichar hanzi,bool traditional){
  ...
  void parseUnihan(FILE *fp,int maxline,sigc::slot<void,gunichar, const char *> action){
    ...
    while (fgets (line,sizeof(line),fp)){//for each line
      ...
      gunichar kanjiuni = parseUPlus(mark);
  ...
  void variantsAction(gunichar kanjiuni, const char *mark){
    ...
    case 'r': //kTraditionalVariant
      ...
      do{
        ++mark;
        gunichar traditional=parseUPlus(mark);
    ...
    case 'i': //kSimplifiedVariant
      ...
      do{
        ++mark;
        gunichar simple=parseUPlus(mark);
        if(simple==0x4E07) {
          wtf("Things!");
        }
...
*/
  static int tonecode(gunichar &c){
    switch(c){
  ...
  static Glib::ustring accentsToDigits(const Glib::ustring &pinyin){
    ...
    if(finaltone){
      flat.append(1,gunichar('0'+finaltone));//cast needed to pick unambiguous overload
    } else {//separate for debug, could just init to 5 and remove the conditional.
      flat.append(1,gunichar('5'));//cast needed to pick unambiguous overload
    }
    return flat;
  }
  ...
  void parseMandarinField(gunichar kanjiuni, const char *mark){
   ...
   * @return single result for hiragana string, sets length to how much of the trailing part of @param hiraganaFragment was actually part of the name (may drop this if we choose a strict lookup)*/
//  gunichar charFor(const Glib::ustring &hiraganaFragment,int &length){
//    init();
//    Storable *entry=KUN.existingChild(hiraganaFragment.c_str());
//    if(entry){
//      length=hiraganaFragment.length();
//      return entry->getNumber<gunichar>();
//    } else {
//      length=0;
//      return 0;
//    }
...
gunichar KanjiDic::charFor(const Glib::ustring &hiraganaFragment, int &/*length*/){
  KanjiDic::init();
