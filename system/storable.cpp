...
__attribute__((init_priority(200))) Storable
Storable::Slash("/");     // probably should tie this a bit harder into PathSep.
...
bool Storable::Delete(TextKey pathname){
  ...
  if (node) {
    if (node->suicide(true)) {
      ...
      storetree("Attempt to delete undeletable node %s", pathname); // root or floating node, can't delete those. Should also not create them!
      // There is no need for floating nodes except as local temps.
      return false;
    }
  } else {
    return false;
...
Storable::Storable(TextKey name, bool isVolatile)
  : isVolatile(isVolatile), type(NotDefined), q(Empty), number(),
  parent(nullptr), index(BadIndex), enumerated(nullptr), name(name){
  if (isVolatile) {
...
Storable::~Storable(){
  ...
  //    storetree("freed more storables than we created!"); //which can happen
  //    if we double free.
  //  }
}
...
Storable &Storable::precreate(TextKey name){
  ...
  Storable &noob(
    *new Storable(name, false));   // only parent needs volatile flag as that stops recursive looking at the children.
  noob.parent = this;
  return noob;
} // Storable::precreate
...
#if 0  //attempt at truly immutable name. Not quite working so going back to violating constness with a cast.
...
void Storable::setEnumerizer(const Enumerated *enumerated){
  if (changed(this->enumerated, enumerated)) {
    if (enumerated) {
      ...
      if ((q >= Parsed) &&
        is(Storable::Textual)) {   // if already has a text value
...
bool Storable::resolve(bool recursively){
  if (is(Storable::Uncertain)) {
    // guess it must be text, attempts to be smarter could not deal with things like a password that happens to be all digits.
...
connection Storable::addChangeWatcher(const SimpleSlot &watcher,
  bool kickme) const {
  if (kickme) {
    watcher();
  }
...
connection Storable::addChangeMother(const SimpleSlot &watcher,
  bool kickme) const {
  if (kickme) {
    watcher();
  }
...
void Storable::reparent(Storable &newparent){
  ...
  if (type == Wad) {
    ...
    dbg("Tried to reparent a scalar node:%s to %s", fullName().c_str(),
      newparent.fullName().c_str());
  }
} // Storable::reparent
...
void Storable::assignFrom(Storable &other){
  ...
  case Textual:
    ...
    break;
  case Wad:
    // implemented for use case of nodes are the backing store of object of identical Stored class we can't trust node order, we must name-match to handle classes built with different versions of software.
...
Cstr Storable::image(void){
  ...
  case Numerical:
    if (enumerated) {
      return enumerated->token(
        number.as<unsigned>());   // don't update text, this is much more efficient since enumerated is effectively static.
    } else {
  ...
  while (progeny.hasNext()) {
    ...
    if (which.isValid()) { // pick node by number.
      ...
      if (autocreate) {
        ...
        return searcher->getChild(
          progeny,
          true);   // must recurse to not copy all the 'is it a number' logic
      }
      return nullptr;
    }
...
Storable &Storable::operator[](unsigned ordinal){
  if (!has(ordinal)) {
    ...
    addChild(""); // better than an NPE so deep in the calling hierarchy that we don't know where it comes from.
    return *wad.last();
  }
...
unsigned StoredListReuser::done(){
  // pointer is quantity that have been done, killer is init to number that exist, if they are equal then all is well.
  for (unsigned killer = node.numChildren(); killer-- > pointer;) {    //#efficient order, no shuffling of ones that will then also be whacked.
    node.remove(killer);
  }
  return pointer;
} // StoredListReuser::done
...
void Storable::Freezer::freezeNode(Storable &node, bool childrenToo,
  bool onlyChildren){
  if (!onlyChildren) {
    node.watchers.gate();
  }
