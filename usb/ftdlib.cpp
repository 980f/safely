...
int &Ftdi::usbReadTtimeout() const {
  return ftdi->usb_read_timeout;
}

int &Ftdi::usbWriteTimeout() const {
  return ftdi->usb_write_timeout;
}

bool Ftdi::has(unsigned bytestoread) const {
  return ftdi && ftdi->readbuffer_remaining >= bytestoread;
}

bool Ftdi::startRead(const ByteScanner &buf) const {
...
bool Ftdi::bitbang_disable() const {
  return ftdi_disable_bitbang(ftdi);
}

int Ftdi::read_pins(unsigned char *pins) const {
  return ftdi_read_pins(ftdi, pins);
}

const char *Ftdi::errorString() const {
...
struct ftdi_context *Ftdi::context() const {
  return ftdi;
}
...
FtdiDeviceList::FtdiDeviceList(Ftdi &context, uint16_t vendor, uint16_t product) : devlist(nullptr) {
  unsigned length = context.findAll(&devlist, vendor, product);
  if (length != BadLength) {
    //presize using length
    allocate(length); //
    for (auto it = devlist; it; it = it->next) {
