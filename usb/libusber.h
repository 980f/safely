...
#include "cstr.h"
#include <libusb/libusb.h> //todo:M elevate libusb folder to an included one.
...
public:
   ...
   * @returns time until next callback required. if Nan or < 0 then ignore value.*/
  MicroSeconds doEvents();

  /** last USB configuration explicitly set by methods of this class */
  int configuration = 0;

  libusb_device_handle *devh = nullptr;

  /** routines that return things have a tendency to hide the errors they encountered. 980F has added code to poke that value where it can be found */
  ...
  /** connection counters mostly for diagnostics, hopefully never exceed one or two, no guard exists against rollover */
  unsigned reconnects = 0;
  unsigned disconnects = 0;

public: //three ways to hook up with a device, depending upon your knowledge of it.
  /** find by vid and pid, if more than one choose nth instance, open it. sets device handle and @returns whether it succeeded. */
