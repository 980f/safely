                             ...
                             const char* description, const char* serial, unsigned int index)
{
    ...
    while ((dev = devs[i++]) != NULL)
    {
        ...
        {
          int libusberror=libusb_open(dev, &ftdi->usb_dev);
            if ( libusberror< 0){
              const char * errortext=libusb_error_name(libusberror);
                ftdi_error_return_free_device_list(-4, errortext,devs);
                                                   //"usb_open() failed", devs);
            }
...
int ftdi_read_pins(struct ftdi_context *ftdi, unsigned char *pins)
{
    ...
    if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_PINS_REQUEST, 0, (uint16_t) ftdi->index, pins, 1, ftdi->usb_read_timeout) != 1)
