...
template<unsigned INDEX, unsigned bias, typename Integrish ,  Integrish ... D> struct pow10Table : pow10Table<INDEX + 1, bias, Integrish,  D..., rpepow10<Integrish>(INDEX+bias)> {};

// termination of recursion at table length, each of the tables generated by this are 10 entries, ~ #bits in type /log2(10)
template<unsigned bias, typename Integrish, Integrish ... D> struct pow10Table<10+bias, bias, Integrish, D...> {
  static constexpr Integrish table[] = {D...};
  enum {size=std::size(table)};
};

//powers of 10 that fit into a 32 bit integer
static constexpr auto Decimal1 =  pow10Table<0,0,uint32_t>::table;
static constexpr size_t Decimal1Size = pow10Table<0,0,uint32_t>::size;

// powers of 10 that fit into a 64 bit integer, but not a 32bit integer.
static constexpr auto Decimal2 = pow10Table<0,10,uint64_t>::table;
static constexpr size_t Decimal2Size = pow10Table<0,10,uint64_t>::size;

#else //keeping around for a bit
//powers of 10 that fit into a 32 bit integer
...
int ilog10(uint32_t value) {
  for (int log = Decimal1Size; log-- > 0;) {
...
int ilog10(uint64_t value) {
  for (int log = Decimal2Size; log-- > 0;) {
...
double dpow10(unsigned uexp) {
  if (uexp < Decimal1Size) {
    return double(Decimal1[uexp]);
  }
  if (uexp < Decimal2Size + Decimal1Size) {
    return double(Decimal2[uexp - Decimal1Size]);
  }
  return 0;
}
 ...
 * was horribly broken until 20july2026.
 * */
