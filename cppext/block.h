...
/** basic block manager. Will add functionality similar to Buffer.h as usages occur */
...
public:
  ...
  Block(unsigned length, Content *buffer, bool ownit = false): length(length),
    buffer(buffer),
    owner(ownit) {
    //#nada
  }
  ...
  /** there are two concepts that might fall under copy construction so don't allow any implicit copying.*/
  Block(Block &) = delete;
