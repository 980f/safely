...
I2C::I2C( unsigned devaddress,unsigned busnumber):Fildes("I2C"),
...
bool I2C::connect(){
  ...
  devname[9]=char('0'+busnumber);
  devname[10]=0;//nulterm

  if(open(devname,O_RDWR)){
    if(ioctl(I2C_SLAVE, address>>1)){//the ioctl wants a 7 bit address, but we use 8 bit addresses as they are what is most ccommonly published and what appear on bus watchers.
